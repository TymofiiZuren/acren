  ...
  return (
    <div className="min-h-screen bg-stone-50 text-stone-950">
      ...
      <aside className="fixed inset-y-0 left-0 hidden w-64 flex-col overflow-y-auto border-r border-stone-200 bg-surface p-5 lg:flex">
        ...
        <p className="px-2 pt-2 text-xs text-stone-500">A little more organised.</p>
        ...
        <div className="mt-auto space-y-5 pt-8"><ThemeSwitch /><div className="rounded-lg bg-stone-100 p-4"><p className="text-sm font-medium">Built around your clients.</p><p className="pt-2 text-xs leading-5 text-stone-600">A clear record today. Less admin tomorrow.</p><Link href="/privacy-centre" className="inline-flex min-h-11 items-center text-xs font-medium text-emerald-900 underline underline-offset-4">Your privacy checklist →</Link></div><div className="border-t border-stone-100 pt-4"><p className="px-3 text-sm font-semibold">Consultant workspace</p><form action={signOut} className="pt-2"><button className="nav-link w-full" type="submit"><LogOutIcon />Sign out</button></form></div></div>
      </aside>
      <main id="main-content" className="lg:pl-64">{children}</main>
    </div>
  );
}
