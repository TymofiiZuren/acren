...
export default async function InvoicePage({params}:{params:Promise<{id:string}>}) {
 ...
 const [invoice,events]=await Promise.all([supabase.from('invoices').select('*').eq('id',id).maybeSingle(),supabase.from('invoice_events').select('id,action,occurred_at').eq('invoice_id',id).order('occurred_at').limit(10)]);
 const {data:i,error}=invoice;
