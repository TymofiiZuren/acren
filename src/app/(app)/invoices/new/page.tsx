...
export default async function NewInvoicePage({searchParams}:{searchParams:Promise<{job?:string}>}) {
 ...
 const [job,profile,rates,existing,recommendation]=await Promise.all([supabase.from('jobs').select('id,client_id,title,status').eq('id',id).maybeSingle(),supabase.from('billing_profiles').select('*').eq('consultant_id',user.id).maybeSingle(),supabase.from('rates').select('*').eq('retired',false).order('created_at',{ascending:false}).limit(200),supabase.from('invoices').select('id').eq('job_id',id).neq('status','void').maybeSingle(),supabase.from('work_recommendations').select('rate_id,quantity').eq('job_id',id).maybeSingle()]);
