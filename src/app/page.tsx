  ...
  return (
    <div className="min-h-screen overflow-hidden bg-stone-100 text-stone-950">
      <header className="mx-auto flex max-w-6xl items-center justify-between px-5 py-6 sm:px-8">
        ...
        <div className="flex flex-wrap items-center justify-end gap-2"><ThemeSwitch /><Link prefetch={false} href="/login" className="button-secondary">Sign in</Link></div>
      </header>
      <main id="main-content" className="mx-auto grid max-w-6xl items-center gap-14 px-5 pb-20 pt-12 sm:px-8 lg:grid-cols-[1.06fr_.94fr] lg:pt-20">
        <section>
          ...
          <Link prefetch={false} href="/signup" className="button-primary mt-9 inline-flex gap-2 px-6">Create an account <ArrowIcon className="size-4" /></Link>
        </section>
      ...
      <footer className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 px-5 py-8 text-sm text-stone-600 sm:px-8"><p>Acren practice management</p><Link prefetch={false} href="/privacy" className="button-quiet">Privacy information</Link></footer>
    </div>
  );
}
