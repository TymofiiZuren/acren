...
export async function ClientDocuments({ clientId, archived, requestedPage }: { clientId: string; archived: boolean; requestedPage?: string }) {
  ...
  return <section id="documents" className="panel space-y-6" aria-labelledby="documents-title">
    <header className="flex flex-wrap items-center justify-between gap-3"><h2 id="documents-title" className="text-xl font-medium">Client documents</h2><span className="eyebrow">Private storage</span></header>
    <p className="rounded-md border border-stone-200 bg-stone-50 p-4 text-sm leading-6"><strong>Storage only — not scheme submission.</strong> Uploading here keeps a copy in the client record. It does not send an application to the Department, record official acceptance, or issue an invoice.</p>
