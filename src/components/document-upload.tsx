...
export function DocumentUpload({ clientId, requestId }: { clientId: string; requestId: string }) {
  ...
  return <form action={action} className="space-y-4" onSubmit={(event) => { if (localError) event.preventDefault(); }}>
    <div className="space-y-2"><label className="field-label" htmlFor="document">Upload a PDF · maximum 750 KB</label>
    ...
    </div><SubmitButton pendingLabel="Uploading document…">Upload to client record</SubmitButton>
  </form>;
}
